...
import com.hybrid.thread.TypingHardThread;
import com.hybrid.thread.TypingNormalThread;
...
public class TypingHardController extends Thread {
    ...
    TypingHardThread hard;
    ThreadGroup group;
    public HashMap<String, TypingHardThread> map = new HashMap<>();
			...
			for(int i=0; i<10; i++){
				...
				Platform.runLater(new Runnable() {
					...
					public void run() {
						...
						hard = new TypingHardThread(group, anchorPane, lblScore, lbl, TypingHardController.this); //EasyThread new
						if(map.containsKey(hard.a1.getText())){
							hard = new TypingHardThread(group, anchorPane, lblScore, lbl, TypingHardController.this);
							map.put(hard.a1.getText(), hard);//map에 키와 값 추가
							hard.sleep = ii;
						}
						else{
							map.put(hard.a1.getText(), hard);//map에 키와 값 추가
		...
		txtField.setOnAction(e1->{
			String s = txtField.getText();
				if(map.containsKey(s)){
					TypingHardThread value = map.get(s);
					txtField.setText("");
