...
public class TypingNormalController extends Thread {
    ...
    TypingNormalThread normal;
    ThreadGroup group;
    public HashMap<String, TypingNormalThread> map = new HashMap<>();
			...
			for(int i=0; i<10; i++){
				...
				Platform.runLater(new Runnable() {
					...
					public void run() {
						...
						normal = new TypingNormalThread(group, anchorPane, lblScore, lbl, TypingNormalController.this); //EasyThread new
						if(map.containsKey(normal.a1.getText())){
							normal = new TypingNormalThread(group, anchorPane, lblScore, lbl, TypingNormalController.this);
							map.put(normal.a1.getText(), normal);//map에 키와 값 추가
							normal.sleep = ii;
						}
						else{
							map.put(normal.a1.getText(), normal);//map에 키와 값 추가
		...
		txtField.setOnAction(e1->{
			String s = txtField.getText();
				if(map.containsKey(s)){
					TypingNormalThread value = map.get(s);
					txtField.setText("");
					...
					if(map.containsKey(normal.a1.getText())){
						...
						map.put(normal.a1.getText(), normal);
					}
					normal.start();
